package com.mclabs.reactive.function.handler;
...
import com.mclabs.reactive.function.model.ProductEvent;
import com.mclabs.reactive.function.repository.ProductRepository;
		...
		return productMono
				.zipWith(existingProductMono,
						(product, existingProduct) -> new Product(existingProduct.getId(), product.getName(),
								product.getPrice()))
				.flatMap(product -> ServerResponse.ok().contentType(APPLICATION_JSON)
						.body(repository.save(product), Product.class).switchIfEmpty(notFound));
	}
