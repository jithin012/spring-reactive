package com.mclabs.reactive.function;
...
import com.mclabs.reactive.function.handler.ProductHandler;
...
//				.build();
		return route().path("/products",
				builder -> builder
						...
						.DELETE("/{id}", handler::deleteProduct).DELETE(handler::deleteAllProducts))
				.build();
	}
}